...
export default function App() {
  ...
  const [econTab, setEconTab] = useState<"client" | "business">("client");
        ...
        <div className="w-full max-w-7xl mx-auto h-full flex flex-col justify-center">
          ...
          <Slide isActive={currentSlide === 11} slideIndex={11} direction={direction}>
            <div id="slide-12-layout" className="w-full flex flex-col space-y-3">
              <div className="border-b border-white/20 pb-2">
                <Heading level={2} className="text-white font-black tracking-tighter text-xl sm:text-2xl md:text-3xl lg:text-4xl">
                  ЭКОНОМИКА: КЛИЕНТ И БИЗНЕС
                </Heading>
                <p className="font-mono text-[9px] sm:text-[10px] text-zinc-500 uppercase tracking-widest mt-1">
                  Переключите вкладку — слева выгода для автора, справа экономика продукта
                </p>
              </div>
              ...
              <div className="relative border border-white/20 bg-[#0a0a0a] rounded-none overflow-visible">
                ...
                <div className="relative border-b border-white/20 flex">
                  {[
                    { id: "client" as const, label: "01 / ДЛЯ КЛИЕНТА", hint: "контент-мейкер" },
                    { id: "business" as const, label: "02 / ДЛЯ БИЗНЕСА", hint: "наш продукт" },
                  ].map((tab) => (
                    <button
                      key={tab.id}
                      type="button"
                      onClick={() => setEconTab(tab.id)}
                      className={`flex-1 px-4 py-3 text-left transition-colors rounded-none border-r border-white/20 last:border-r-0 ${
                        econTab === tab.id
                          ? "bg-[#FF4A22] text-white"
                          : "bg-black text-zinc-400 hover:text-white hover:bg-[#151515]"
                      }`}
                    >
                      <span className="font-mono text-[10px] sm:text-xs font-black uppercase tracking-widest block">
                        {tab.label}
                      </span>
                      <span className={`font-mono text-[8px] uppercase tracking-wide ${econTab === tab.id ? "text-white/80" : "text-zinc-600"}`}>
                        {tab.hint}
                      </span>
                    </button>
                  ))}
                  ...
                  {econTab === "client" ? (
                    <>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="relative border-2 border-[#FF4A22] bg-black p-4 flex flex-col justify-between min-h-[120px] rounded-none">
                          <div className="absolute top-0 left-0 w-full h-0.5 bg-[#FF4A22]" />
                          <div className="flex items-start justify-between gap-2">
                            <span className="font-mono text-[8px] sm:text-[9px] font-black text-zinc-500 uppercase tracking-widest">
                              ВЫГОДА КЛИЕНТА
                            </span>
                            <TrendingUp className="w-4 h-4 text-[#FF4A22] shrink-0" strokeWidth={2.5} />
                          </div>
                          <div className="pt-2">
                            <div className="text-3xl sm:text-4xl font-mono font-black text-[#FF4A22] tracking-tighter leading-none">
                              +75 000 ₽
                            </div>
                            <p className="text-[10px] sm:text-xs text-zinc-300 font-bold mt-1">
                          ...
                          <span className="font-mono text-[7px] sm:text-[8px] text-zinc-500 uppercase tracking-widest font-black mt-3">
                            экономия + рост рекламы
                          </span>
                        </div>
                        <div className="relative border border-white/15 bg-[#111111] p-4 flex flex-col justify-between min-h-[120px] rounded-none">
                          <div className="flex items-start justify-between gap-2">
                            <span className="font-mono text-[8px] sm:text-[9px] font-black text-zinc-500 uppercase tracking-widest">
                              СТОИМОСТЬ СЕРВИСА
                            </span>
                            <Wallet className="w-4 h-4 text-[#FF4A22] shrink-0 opacity-80" strokeWidth={2.5} />
                          </div>
                          <div className="pt-2">
                            <div className="text-3xl sm:text-4xl font-mono font-black text-white tracking-tighter leading-none">
                              990 ₽
                            </div>
                            <p className="text-[10px] sm:text-xs text-zinc-300 font-bold mt-1">
                              подписка в месяц
                      ...
                      <div className="border border-white/15 bg-black p-4 rounded-none">
                        ...
                        <div className="space-y-2">
                          {[
                            { val: "−50 000 ₽", label: "ОТКАЗ ОТ КОПИРАЙТЕРА", desc: "не платит редактору или фрилансеру", tone: "text-zinc-400" },
                            { val: "+25 000 ₽", label: "БОЛЬШЕ РЕКЛАМЫ", desc: "×3 постов → больше показов и дохода", tone: "text-zinc-200" },
                            { val: "+75 000 ₽", label: "ИТОГО В МЕСЯЦ", desc: "клиент в плюсе, даже с подпиской 990 ₽", tone: "text-[#FF4A22]" },
                          ].map((row) => (
                            <div key={row.label} className="flex items-start justify-between gap-3 border-b border-white/10 pb-2 last:border-0 last:pb-0">
                              <div>
                                <span className="font-mono text-[8px] sm:text-[9px] font-black text-zinc-500 uppercase tracking-widest block">
                                  {row.label}
                                </span>
                                <span className="text-[10px] text-zinc-500 font-bold">{row.desc}</span>
                              </div>
                              <span className={`font-mono text-sm sm:text-lg font-black tracking-tighter shrink-0 ${row.tone}`}>
                                {row.val}
                              </span>
                            </div>
                          ))}
                      ...
                      <p className="font-mono text-[9px] sm:text-[10px] text-zinc-400 font-bold uppercase tracking-wide border-l-4 border-[#FF4A22] pl-3">
                        Клиент платит 990 ₽ — получает инструмент, который может приносить десятки тысяч рублей с рекламы.
                      </p>
                    </>
                  ) : (
                    <>
